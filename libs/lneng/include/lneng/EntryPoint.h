...
#define LNENG_DEFINE_ENTRYPOINT(GameClass)                               \
  ...
  auto SDL_AppIterate(void* appState) -> SDL_AppResult {                 \
    try {                                                                \
      ...
      SDL_Log("FATAL (update): %s", e.what());                           \
      return SDL_APP_FAILURE;                                            \
    } catch (...) {                                                      \
      SDL_Log("FATAL (update): unknown exception");                      \
      return SDL_APP_FAILURE;                                            \
    }                                                                    \
  }                                                                      \
  auto SDL_AppEvent(void* appState, SDL_Event* event) -> SDL_AppResult { \
    try {                                                                \
      ...
      SDL_Log("FATAL (event): %s", e.what());                            \
      return SDL_APP_FAILURE;                                            \
    } catch (...) {                                                      \
      SDL_Log("FATAL (event): unknown exception");                       \
      return SDL_APP_FAILURE;                                            \
    }                                                                    \
  }                                                                      \
